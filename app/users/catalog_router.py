...
async def page_catalog(call: CallbackQuery, session_without_commit: AsyncSession):
    await call.answer(text='Подгружаем каталог....')
...
async def page_catalog_products(call: CallbackQuery, session_without_commit: AsyncSession):
    ...
    if count_products > 0:
        await call.message.answer(text=f'В данной категории {count_products} продукт(ов):')
        ...
        for product in products_category:
            ...
            await call.message.answer(
                text=product_text,
                reply_markup=products_kb(product.id, product.price),
                parse_mode="HTML"
            )

    else:
        await call.answer(text='В данной категории нет товаров', show_alert=True)
